    ...
    def download_batch(batch):
        ...
        try:
            ...
            for (
                ...
                dt_be = _OMERO_PIXEL_DTYPES.get(pixels_type)
                if dt_be is None:
                    raise ValueError(f"Unsupported OMERO pixel type: {pixels_type}")
                ...
                for t in range(sizeT):
                    ...
                    if args.numpy:
                        np.save(os.path.join(dir, f"s{image_id}_t{t}.npy"), arr)
                    else:
                        imwrite(os.path.join(dir, f"s{image_id}_t{t}.tiff"), arr)
        finally:
